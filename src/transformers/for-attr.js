...
var fixPropertyLooping = require('../util/fixPropertyLooping');
...
exports.transform = function(el) {
    ...
    let forEachProps = attributeParser.parse(stringValue, {
        each: { type: 'custom' },
        ...
        errorHandler: function (message) {
            throw new Error('Invalid for attribute of "' + forAttr.name + '". Error: ' + message);
        }
    });

    let final = fixPropertyLooping(forEachProps.each);

    if (forEachProps.separator || forEachProps.iterator || forEachProps.statusVar) {
        final += ' |';
