...
function getConditionalExpression(expression) {
    ...
    function getExpression(part) {
        ...
        parse(part, {
            text: function (text) {
                expressionParts.push(stringify(text, { useSingleQuote: true }));
            },
...
ExpressionParserHelper.prototype = {
    _invokeCallback: function (name, value, escapeXml) {
        if (!this.listeners[name]) {
            throw new Error(name + ' not allowed: ' + value);
        }
...
parse = function (str, listeners, options) {
    ...
    outer:
        while ((startMatches = startRegExp.exec(str))) {
            if (startMatches[0].startsWith('\\\\')) {
            ...
            } else if (startMatches[0].startsWith('\\')) {
                // \${
                /*
                ...
                throw new Error('Illegal state. Unexpected start token: ' + startMatches[0]);
            }
