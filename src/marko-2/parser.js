...
const taglibs = require("./taglibs");
const handleBinaryOperators = require("./handleBinaryOperators");
  ...
  var parser = new htmlparser.Parser(
    {
      ...
      onprocessinginstruction: function(name, data) {
        if (data.startsWith("!")) {
          var doctypeNode = builder.documentType(
            builder.literal(data.substring(1))
          );
          currentParent.appendChild(doctypeNode);
        } else if (data.startsWith("?") && data.endsWith("?")) {
          var declarationNode = builder.declaration(
            builder.literal(data.substring(1, data.length - 1))
          );
